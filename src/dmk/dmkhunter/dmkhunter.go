...
func startScan(rootPath string) {
	...
	for _,f := range files {
		...
		md5list := scanPath(path)
		saveToFile(VARDIR + "/newstamp.dat", &md5list)
	}
...
func checkFilelist(filename string) ([]string, []string) {
	...
	for j := 0; j < fileCount; {
		...
		case <-time.After(time.Second):
			log.Fatal("Timeout on read filelist")
		}
	}
...
func splitFileList(filePath string, files chan string, ignores chan string, quitChan chan int) {
	...
	default:
		log.Println("could not read line", filePath)
	}
}
...
func saveToFile(filePath string, data *Md5List) {
